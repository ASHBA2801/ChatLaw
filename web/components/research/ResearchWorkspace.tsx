...
export default function ResearchWorkspace({ initialQuery }: { initialQuery: string }) {
  ...
  const [cases, setCases] = useState<LandmarkCase[]>([]);
  ...
  async function runSearch(value: string) {
    ...
    try {
      ...
      setNoRelevant(response.no_relevant_context && landmarkHits.length === 0);
      router.replace(`/research?q=${encodeURIComponent(value)}`, { scroll: false });
    } catch (err) {
      setResults([]);
      // Landmark cases still usable offline from local curated data.
      setNoRelevant(landmarkHits.length === 0);
  ...
  const filteredCases = useMemo(() => {
    ...
    return cases.filter((item) =>
      [item.title, item.citation, item.court, item.summary, ...item.topics].join(" ").toLowerCase().includes(q),
    );
  }, [cases, filter]);
    ...
    <div className="space-y-6">
      ...
      {searched && !loading && showCases && filteredCases.length > 0 ? (
        <section className="space-y-3">
          <div>
            <h2 className="text-lg font-semibold">Landmark cases</h2>
            <p className="mt-1 text-xs text-[var(--ink-muted)]">
              Curated verified judgments with public citations. ChatLaw does not invent case holdings.
            </p>
          </div>
