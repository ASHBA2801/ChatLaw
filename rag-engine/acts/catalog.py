...
class ActCatalogEntry:
    ...
    official_url: str
    status: str  # in_force | repealed | pending_commencement
    ...
    def from_dict(cls, data: Mapping[str, object]) -> "ActCatalogEntry":
        return cls(
            ...
            domain=str(data.get("domain", "general_legal")),
            jurisdiction=str(data.get("jurisdiction", "India")),
            document_type=str(data.get("document_type", "statute")),
...
class ActCatalog:
    ...
    def get_by_domain(self, domain: str) -> tuple[ActCatalogEntry, ...]:
        return tuple(e for e in self._entries_by_id.values() if e.domain == domain)
